...
export class TVLivestreamsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.socketService.syncMessages("play video").subscribe(msg => {
      ...
      /**update video style and hide buttons if another "device" called playVideo() */
      if(msg.message.device != 2) {
    ...
    if(this.globals.currentVideoPlayer == 2){
  ...
  public pause_play(number){
    ...
    if(this.globals.currentVideo == number && this.globals.currentVideoPlayer == 2){
      /**visibilities */
      if(this.globals.paused == 0 ){
